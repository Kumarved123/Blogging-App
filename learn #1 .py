...
def dashboard():
    ...
    if request.method== 'POST':
        username = request.form.get('unmae')
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        ...
        if sno==0:
            post = Posts(title=title, tagline = tline, slug = slug, content= content, img_file= img_file, date= datetime.now(),sno =sno)
            db.session.add(post)
            db.session.commit()
